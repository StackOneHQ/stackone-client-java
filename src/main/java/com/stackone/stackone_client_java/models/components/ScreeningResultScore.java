/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
package com.stackone.stackone_client_java.models.components;

import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonIgnore;
import com.fasterxml.jackson.annotation.JsonInclude.Include;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.stackone.stackone_client_java.utils.Utils;
import java.lang.Override;
import java.lang.String;
import org.openapitools.jackson.nullable.JsonNullable;

/**
 * ScreeningResultScore
 * 
 * <p>Score information
 */
public class ScreeningResultScore {
    /**
     * Score label
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("label")
    private JsonNullable<String> label;

    /**
     * Score value
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("value")
    private JsonNullable<String> value;

    /**
     * Minimum score value
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("min")
    private JsonNullable<String> min;

    /**
     * Maximum score value
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("max")
    private JsonNullable<String> max;

    @JsonCreator
    public ScreeningResultScore(
            @JsonProperty("label") JsonNullable<String> label,
            @JsonProperty("value") JsonNullable<String> value,
            @JsonProperty("min") JsonNullable<String> min,
            @JsonProperty("max") JsonNullable<String> max) {
        Utils.checkNotNull(label, "label");
        Utils.checkNotNull(value, "value");
        Utils.checkNotNull(min, "min");
        Utils.checkNotNull(max, "max");
        this.label = label;
        this.value = value;
        this.min = min;
        this.max = max;
    }
    
    public ScreeningResultScore() {
        this(JsonNullable.undefined(), JsonNullable.undefined(), JsonNullable.undefined(),
            JsonNullable.undefined());
    }

    /**
     * Score label
     */
    @JsonIgnore
    public JsonNullable<String> label() {
        return label;
    }

    /**
     * Score value
     */
    @JsonIgnore
    public JsonNullable<String> value() {
        return value;
    }

    /**
     * Minimum score value
     */
    @JsonIgnore
    public JsonNullable<String> min() {
        return min;
    }

    /**
     * Maximum score value
     */
    @JsonIgnore
    public JsonNullable<String> max() {
        return max;
    }

    public static Builder builder() {
        return new Builder();
    }


    /**
     * Score label
     */
    public ScreeningResultScore withLabel(String label) {
        Utils.checkNotNull(label, "label");
        this.label = JsonNullable.of(label);
        return this;
    }

    /**
     * Score label
     */
    public ScreeningResultScore withLabel(JsonNullable<String> label) {
        Utils.checkNotNull(label, "label");
        this.label = label;
        return this;
    }

    /**
     * Score value
     */
    public ScreeningResultScore withValue(String value) {
        Utils.checkNotNull(value, "value");
        this.value = JsonNullable.of(value);
        return this;
    }

    /**
     * Score value
     */
    public ScreeningResultScore withValue(JsonNullable<String> value) {
        Utils.checkNotNull(value, "value");
        this.value = value;
        return this;
    }

    /**
     * Minimum score value
     */
    public ScreeningResultScore withMin(String min) {
        Utils.checkNotNull(min, "min");
        this.min = JsonNullable.of(min);
        return this;
    }

    /**
     * Minimum score value
     */
    public ScreeningResultScore withMin(JsonNullable<String> min) {
        Utils.checkNotNull(min, "min");
        this.min = min;
        return this;
    }

    /**
     * Maximum score value
     */
    public ScreeningResultScore withMax(String max) {
        Utils.checkNotNull(max, "max");
        this.max = JsonNullable.of(max);
        return this;
    }

    /**
     * Maximum score value
     */
    public ScreeningResultScore withMax(JsonNullable<String> max) {
        Utils.checkNotNull(max, "max");
        this.max = max;
        return this;
    }

    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        ScreeningResultScore other = (ScreeningResultScore) o;
        return 
            Utils.enhancedDeepEquals(this.label, other.label) &&
            Utils.enhancedDeepEquals(this.value, other.value) &&
            Utils.enhancedDeepEquals(this.min, other.min) &&
            Utils.enhancedDeepEquals(this.max, other.max);
    }
    
    @Override
    public int hashCode() {
        return Utils.enhancedHash(
            label, value, min,
            max);
    }
    
    @Override
    public String toString() {
        return Utils.toString(ScreeningResultScore.class,
                "label", label,
                "value", value,
                "min", min,
                "max", max);
    }

    @SuppressWarnings("UnusedReturnValue")
    public final static class Builder {

        private JsonNullable<String> label = JsonNullable.undefined();

        private JsonNullable<String> value = JsonNullable.undefined();

        private JsonNullable<String> min = JsonNullable.undefined();

        private JsonNullable<String> max = JsonNullable.undefined();

        private Builder() {
          // force use of static builder() method
        }


        /**
         * Score label
         */
        public Builder label(String label) {
            Utils.checkNotNull(label, "label");
            this.label = JsonNullable.of(label);
            return this;
        }

        /**
         * Score label
         */
        public Builder label(JsonNullable<String> label) {
            Utils.checkNotNull(label, "label");
            this.label = label;
            return this;
        }


        /**
         * Score value
         */
        public Builder value(String value) {
            Utils.checkNotNull(value, "value");
            this.value = JsonNullable.of(value);
            return this;
        }

        /**
         * Score value
         */
        public Builder value(JsonNullable<String> value) {
            Utils.checkNotNull(value, "value");
            this.value = value;
            return this;
        }


        /**
         * Minimum score value
         */
        public Builder min(String min) {
            Utils.checkNotNull(min, "min");
            this.min = JsonNullable.of(min);
            return this;
        }

        /**
         * Minimum score value
         */
        public Builder min(JsonNullable<String> min) {
            Utils.checkNotNull(min, "min");
            this.min = min;
            return this;
        }


        /**
         * Maximum score value
         */
        public Builder max(String max) {
            Utils.checkNotNull(max, "max");
            this.max = JsonNullable.of(max);
            return this;
        }

        /**
         * Maximum score value
         */
        public Builder max(JsonNullable<String> max) {
            Utils.checkNotNull(max, "max");
            this.max = max;
            return this;
        }

        public ScreeningResultScore build() {

            return new ScreeningResultScore(
                label, value, min,
                max);
        }

    }
}
